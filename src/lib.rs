...
pub struct Connection {
    ...
    sps: Option<Bytes>,
    pps: Option<Bytes>,
    tx: mpsc::Sender<AuPayload>,
    txc: broadcast::Sender<()>,
}

impl Connection {
    pub fn new(id: i32, tx: mpsc::Sender<AuPayload>, txc: broadcast::Sender<()>) -> Self {
        Connection {
            id,
            session: None,
            state: State::Waiting,
        ...
        loop {
                ...
                .map_err(|x| format!("Connection {}: Failed to process bytes: {:?}", self.id, x))?
            {
                ...
                HandshakeProcessResult::Completed {
                    ...
                    stream.write_all(&response_bytes).await?;
                    spawn(self.start_connection_manager(stream, remaining_bytes, txk, streams));
                    return Ok(());
                }
            }
        }
                ...
                match self.state {
                    State::Publishing {
                        ...
                        if let Some(metadata) = &self.metadata {
                            streams
                                .add_rtmp_metadata(stream_key.clone(), metadata.clone())
                                .await;
                        }
        ...
        match event {
                ...
                if let Ok(mut au) = flv::extract_au(data, timestamp.value as i64) {
                    match au.kind {
                        ...
                        AuKind::AVC => {
                            au.key = is_key_frame;
                            au.path = None;
                            au.sps = self.sps.clone();
                            au.pps = self.pps.clone();
                                ...
                                Err(e) => {
                                    match e {
                                        ...
                                        tokio::sync::mpsc::error::TrySendError::Closed(_) => {
                                            error!("Failed to send AuPayload: Receiver has been dropped");
                                            return Ok(ConnectionAction::Disconnect);
                                        }
                                    }
                                }
            ...
            ServerSessionEvent::AudioDataReceived {
                ...
                if let Some(m) = &self.metadata {
                    match self.tx.try_send(AuPayload {
                        kind: AuKind::AAC,
                        data: Some(data),
                        dts: Some(Duration::milliseconds(timestamp.value as i64)),
                        pts: Some(Duration::milliseconds(timestamp.value as i64)),
                        path: None,
                        audio_codec_id: Some(m.audio_codec_id.unwrap_or(0) as u8),
                        audio_channels: Some(m.audio_channels.unwrap_or(0) as u8),
